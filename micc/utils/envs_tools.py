...
import numpy as np
import torch
from micc.envs.env_wrappers import ShareSubprocVecEnv, ShareDummyVecEnv
...
def make_train_env(env_name, seed, n_threads, env_args):
    ...
    def get_env_fn(rank):
        def init_env():
            if env_name == "smac":
                from micc.envs.smac.StarCraft2_Env import StarCraft2Env
...
def get_num_agents(env, env_args, envs):
    ...
    if env == "smac":
        from micc.envs.smac.smac_maps import get_map_params
